  ...
  return (
    <div className="relative w-full h-full aspect-square max-w-md mx-auto lg:max-w-none">
      ...
      <div className="absolute -bottom-4 -right-4 sm:-bottom-6 sm:-right-6 bg-white rounded-xl p-3 sm:p-4 shadow-xl border border-blue-100">
        ...
        <div className="text-xl sm:text-2xl font-bold text-blue-700">98%</div>
      </div>
    ...
    <div className="flex flex-col font-['Inter'] antialiased">
      ...
      <section id="hero" className="relative w-full min-h-screen px-4 sm:px-6 lg:px-24 py-16 lg:py-24 bg-gradient-to-br from-[#f0f5ff] to-[#e6ecff] rounded-3xl overflow-hidden flex flex-col lg:flex-row items-center">
        ...
        <div className="relative z-10 w-full lg:w-1/2 mb-10 lg:mb-0">
          ...
          >
            <Link to="/contact">
              <button className="px-5 py-2 sm:px-6 sm:py-3 rounded-3xl bg-gradient-to-r from-[#2562EA] to-[#6B8DFF] text-white font-semibold text-base sm:text-lg transform hover:-translate-y-1 hover:scale-105 transition-all duration-300">
                Get Started
              </button>
            </Link>
            <Link to="/consultation">
              <button className="px-5 py-2 sm:px-6 sm:py-3 rounded-3xl bg-gradient-to-r from-orange-500 to-orange-600 text-white font-semibold text-base sm:text-lg transform hover:-translate-y-1 hover:scale-105 transition-all duration-300">
      ...
      {extraSections.map((section, idx) => {
        ...
        if (section.heading.includes("Which Types of Software")) {
          ...
          return (
            <section key={idx} id={sectionId} className="px-4 sm:px-6 lg:px-24 py-16 sm:py-20 bg-white">
              <SectionHeading
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-6 sm:p-8">
                        <div className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full bg-[#E5F0FF] group-hover:bg-[#2562EA] transition-colors mb-4 z-10">
                          {IconComponent ? (
                            <IconComponent className="h-5 w-5 sm:h-6 sm:w-6 text-[#2562EA] group-hover:text-white" />
                          ) : (
                            <Icons.HardDrive className="h-5 w-5 sm:h-6 sm:w-6 text-[#2562EA] group-hover:text-white" />
                          )}
                        </div>
        ...
        if (section.heading.includes("The Software Development Lifecycle")) {
          ...
          return (
            <section key={idx} id={sectionId} className="px-4 sm:px-6 lg:px-24 py-16 sm:py-20 bg-gray-50">
              <SectionHeading
                text={section.heading}
                gradientFrom="#2562EA"
                gradientTo="#6B8DFF"
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-12">
                {steps.map((step, i) => (
                  <motion.div
                    key={i}
                    className="group relative p-5 sm:p-6 bg-[#2562EA] rounded-3xl flex flex-col items-center text-center transition-transform duration-400 hover:scale-105 overflow-hidden"
                  ...
                  >
                    ...
                    <div className="relative w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center rounded-full bg-[#1c48a8] mb-4 sm:mb-5 group-hover:rotate-6 transition-transform duration-500">
                    ...
                    <p className="font-semibold text-white text-base sm:text-lg relative z-10">{step}</p>
                  </motion.div>
                ))}
              </div>
            </section>
          ...
          <section key={idx} id={sectionId} className="px-4 sm:px-6 lg:px-24 py-16 sm:py-20 bg-white">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
              {section.items?.map((item, i) => (
                <motion.div
                  key={i}
                  className="bg-gradient-to-br from-[#f0f5ff] via-white to-[#e6ecff] rounded-3xl shadow-md p-6 sm:p-8 flex flex-col items-center text-center transition-transform hover:scale-105 hover:shadow-xl group"
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.1, duration: 0.5 }}
                  viewport={{ once: true }}
                >
                  <div className="w-14 h-14 sm:w-16 sm:h-16 flex items-center justify-center rounded-full bg-[#E5F0FF] group-hover:bg-[#2562EA] mb-4 sm:mb-5 transition-colors">
                    <Icons.CheckCircle2 className="w-7 h-7 sm:w-8 sm:h-8 text-[#2562EA] group-hover:text-white" />
                  </div>
      ...
      {/* FAQ SECTION */}
      {faqs && faqs.length > 0 && (
        <section id="faqs" className="px-4 sm:px-6 lg:px-24 py-16 sm:py-20 bg-gray-50">
          <SectionHeading
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <Accordion type="single" collapsible className="space-y-4 sm:space-y-6 max-w-4xl mx-auto">
              {faqs.map((faq, idx) => (
                <AccordionItem
                  key={idx}
                  value={`faq-${idx}`}
                ...
                >
                  <AccordionTrigger className="flex justify-between items-center text-base sm:text-lg lg:text-xl font-semibold text-gray-800 p-4 sm:p-6 pr-4 hover:bg-gradient-to-r hover:from-[#2562EA]/10 hover:to-[#6B8DFF]/10 transition-all duration-300 rounded-3xl group">
                    {faq.question}
                  </AccordionTrigger>
                  <AccordionContent className="text-gray-700 p-4 sm:p-6 border-t border-gray-100 bg-[#f0f5ff] text-sm sm:text-base leading-relaxed rounded-b-3xl">
                    {faq.answer}
                  </AccordionContent>
                </AccordionItem>
              ))}
      ...
      {testimonials && testimonials.length > 0 && (
        <section id="testimonials" className="px-4 sm:px-6 lg:px-24 py-16 bg-white">
          <SectionHeading
            ...
            gradientFrom="#2562EA"
            gradientTo="#6B8DFF"
            className="mb-12 text-center text-2xl sm:text-3xl"
          />
          <div className="grid md:grid-cols-2 gap-8 sm:gap-10">
            {testimonials.map((t, idx) => (
              <motion.div
                ...
                viewport={{ once: true }}
              >
                <Icons.Quote className="absolute top-8 right-8 w-16 h-16 sm:w-24 sm:h-24 text-[#E5F0FF] opacity-70 rotate-180 -z-0 transform group-hover:scale-110 transition-transform duration-500" />
      ...
      <section id="other-services" className="px-4 sm:px-6 lg:px-24 pb-16 sm:pb-24">
        <SectionHeading
          ...
          gradientFrom="#2562EA"
          gradientTo="#6B8DFF"
          className="mb-8 sm:mb-10 text-xl sm:text-3xl text-center"
        />
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {otherServices.map((service, idx) => (
            <motion.a
              key={idx}
              href={service.path}
              className="block p-4 sm:p-6 rounded-2xl border border-gray-200 bg-white hover:border-[#6B8DFF] transition-all duration-300 text-center relative overflow-hidden group"
              initial={{ opacity: 0, y: 25 }}
              whileInView={{ opacity: 1, y: 0 }}
